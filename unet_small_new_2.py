...
def train_model(total_epochs, save_dir,train_images,train_labels,val_images,val_labels):
    ...
    model = models.unet_2d((1024, 1280,3), [4, 6, 12, 32, 32], n_labels=1,
...
#     model.compile(loss=IOU_loss, optimizer=keras.optimizers.SGD(learning_rate=1e-4))
    model.compile(loss=IOU_loss, optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001, amsgrad = True))
