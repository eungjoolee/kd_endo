...
#     _sub_index = 1
    _img_feature = np.zeros((_img.shape[0],1024,1280,2))
...
def train_model(total_epochs, save_dir,train_images,train_labels,val_images,val_labels):
    ...
    logging.basicConfig(filename='train_unet_status.log', level=logging.DEBUG, format=LOG_FORMAT)
    
    
    # define student model and GTA, the GTAs have the same setting with student model


    s_model = models.unet_2d((1024, 1280,3), [4, 6, 12, 32, 32], n_labels=1,
                      stack_num_down=1, stack_num_up=1,
                      activation='GELU', output_activation='Sigmoid', 
                      batch_norm=True, pool='max', unpool='nearest', name='unet')

    t_s_model_1 = models.unet_2d((1024, 1280,3), [4, 6, 12, 32, 32], n_labels=1,
                      stack_num_down=1, stack_num_up=1,
                      activation='GELU', output_activation='Sigmoid', 
                      batch_norm=True, pool='max', unpool='nearest', name='unet')
    
    t_s_model_2 = models.unet_2d((1024, 1280,3), [4, 6, 12, 32, 32], n_labels=1,
                      stack_num_down=1, stack_num_up=1,
                      activation='GELU', output_activation='Sigmoid', 
                      batch_norm=True, pool='max', unpool='nearest', name='unet')
    ...
    t_model = models.unet_2d((1024, 1280,3), [64, 128, 256, 512, 512], n_labels=1,
                      stack_num_down=1, stack_num_up=1,
                      activation='GELU', output_activation='Sigmoid', 
                      batch_norm=True, pool='max', unpool='nearest', name='unet')
    ...
    t_transunet.load_weights(t_latest)
    ...
    s_unet.load_weights(u_latest)
    t_self_1_unet.load_weights(u_latest)
    t_self_2_unet.load_weights(u_latest)
    ...
    losses_unet = {"unet_output": st_loss, "unet_output_activation": IOU_loss}
